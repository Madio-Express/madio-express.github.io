      ...
      bricksBuilder.create('payment', containerId, {
        ...
        callbacks: {
          ...
          onSubmit: function (param) {
            ...
            var deviceSessionId = '';
            try {
              // 1. MercadoPago SDK instance method (preferred for Payment Brick)
              if (mp && typeof mp.getDeviceProfile === 'function') {
                var profile = mp.getDeviceProfile();
                if (profile && profile.id) deviceSessionId = profile.id;
              }
              // 2. MercadoPago SDK deviceProfileId property
              if (!deviceSessionId && mp && mp.deviceProfileId) {
                deviceSessionId = mp.deviceProfileId;
              }
              // 3. Hidden input generated by the SDK fingerprint script
              if (!deviceSessionId) {
                var deviceInput = document.getElementById('deviceId');
                if (deviceInput && deviceInput.value) deviceSessionId = deviceInput.value;
              }
              // 4. Global variable set by MercadoPago.js v2
              if (!deviceSessionId && window.MP_DEVICE_SESSION_ID) {
                deviceSessionId = window.MP_DEVICE_SESSION_ID;
              }
              // 5. mp internal fingerprint (undocumented but used by Bricks internally)
              if (!deviceSessionId && mp && mp._instanceProperties && mp._instanceProperties.deviceProfileId) {
                deviceSessionId = mp._instanceProperties.deviceProfileId;
              }
              // 6. Cookie fallback
              if (!deviceSessionId) {
